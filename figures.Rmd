...
```{r 3-geom_pointinterval_quantiles, fig.height = 2, fig.width = 2.5}
p = data.frame(a = dist_normal(0, 1)) |>
  ggplot(aes(xdist = a)) +
  stat_pointinterval(.width = .95, justification = 0.6) +
  scale_y_continuous(breaks = NULL) +
  scale_x_continuous(breaks = NULL) +
  label_(label = "median(italic(A))", x = 0, y = 0.19, hjust = 0.5, parse = TRUE) +
  arrow_(x = -1.96, y = -0.27, yend = -0.06) +
  label_(label = "{italic(F)[italic(A)]^{-1}}~bgroup('(', over(1 - gamma, 2), ')')", x = -2.05, y = -0.375, hjust = 0, parse = TRUE) +
  arrow_(x = 1.96, y = -0.2, yend = -0.06) +
  label_(label = "{italic(F)[italic(A)]^{-1}}~bgroup('(', over(1 + gamma, 2), ')')", x = 2.3, y = -0.375, hjust = 1, parse = TRUE) +
  labs(y = NULL)
