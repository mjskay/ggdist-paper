...
```{r slab_two_groups, fig.width = 2.5, fig.height = 2}
two_a = data.frame(
  a = c(6, 1),
  sigma = c(1.8, 1.3),
  group = factor(c(1, 2))
...
p = two_a |>
  ggplot(aes(xdist = dist_normal(a, sigma), y = group)) +
  stat_slab() +
  # stat_spike(
  ...
  label_(x = 13.5, y = c(1.9, 2.9), label = "max(italic(f[A](x)))", parse = TRUE, hjust = 1, vjust = 0.8) +
