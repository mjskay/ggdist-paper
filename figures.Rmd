...
slabinterval_teaser = (dists_header_plot / dists_plot) + plot_layout(heights = c(1.2, 8))
slabinterval_teaser
```
...
p = data.frame(a = dist_normal(0, 1)) |>
  ...
  scale_y_continuous(breaks = NULL) +
  scale_x_continuous(breaks = NULL, limits = c(-2.5, 2.5)) +
  label_(label = "Normal(italic(mu), italic(sigma))", x = 0, y = 0.15, hjust = 0.5, vjust = 1, parse = TRUE) +
  labs(y = NULL, x = NULL)
...
p = data.frame(a = dist_student_t(6, 0, 1)) |>
  ...
  scale_y_continuous(breaks = NULL) +
  scale_x_continuous(breaks = NULL, limits = c(-2.5, 2.5)) +
  label_(label = "italic(t[nu])(italic(mu), italic(sigma))", x = 0, y = 0.35, hjust = 0.5, vjust = 1, parse = TRUE) +
  labs(y = NULL, x = NULL)
...
p = two_a |>
  ...
  stat_slab() +
  stat_pointinterval(.width = 0.95, linewidth = 0.5) +
...
p = two_a |>
  ggplot(aes(xdist = dist_student_t(nu, mu, sigma), y = group)) +
  stat_eye(aes(thickness = ifelse(.width <= .95, log(!!p_(x)), NA)), normalize = "groups", .width = .95, linewidth = 0.5) +
  scale_x_continuous(breaks = NULL, limits = c(-3, 14)) +
  scale_y_discrete(expand = expansion(0.15)) +
  guides(alpha = "none") +
  labs(x = NULL)
...
```{r slab_fuzzygram, fig.width = 2.5, fig.height = 1.5}
p = two_a |>
  ggplot(aes(xdist = dist_student_t(nu, mu, sigma), y = group)) +
...
```{r dots_two, fig.width = 2.5, fig.height = 2}
p = two_a_zero |>
  ggplot(aes(xdist = dist_student_t(nu, mu, sigma), y = group)) +
...
```{r dots_emoji, fig.width = 2.5, fig.height = 2}
p = two_a_zero |>
  ggplot(aes(xdist = dist_student_t(nu, mu, sigma), y = group)) +
  geom_vline(xintercept = 0, color = "gray85", linewidth = 0.25) +
  stat_dots(aes(shape = after_stat(x > 0)), quantiles = 30, stackratio = 1.1, dotsize = 0.9) +
  scale_x_continuous(breaks = seq(-2, 8, by = 2), limits = c(-2.75, 8.9)) +
  scale_y_discrete(expand = expansion(0.15)) +
  scale_shape_manual(
    values = c("FALSE" = "😧", "TRUE" = "🙂"), 
    guide = guide_legend(reverse = TRUE, title.hjust = 0.92, override.aes = list(linewidth = 4))
  ) +
  labs(x = NULL) +
...
```{r 3-jacobian-incorrect, fig.width=2.5, fig.height = 1.5}
p = ggplot() +
  stat_function(fun = \(x) dlnorm(x, 0, 1), n = 1000, geom = "area", fill = "gray75") +
  geom_vline(xintercept = 1, color = label_color, linewidth = 0.5) +
  label_(x = 1.35, y = 0.625, label = "median???") +
...
```{r 3-jacobian-correct, fig.width=2.5, fig.height = 1.5}
p = ggplot() +
  stat_slab(aes(xdist = dist_lognormal(0, 1)), scale = 1, fill = "gray75") +
  geom_vline(xintercept = 1, color = label_color, linewidth = 0.5) +
  label_(x = 1.75, y = 0.95, label = "median :)") +
  scale_x_log10(limits = 10^c(-2,2), breaks = 10^(-2:2), labels = as.character) +
  scale_y_continuous(breaks = NULL) +
  labs(x = NULL, y = NULL)
