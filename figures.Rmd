...
p = data.frame(a = dist_normal(0, 1)) |>
  ...
  scale_y_continuous(breaks = NULL) +
  scale_x_continuous(breaks = NULL, limits = c(-2.5, 2.5)) +
  scale_color_brewer(palette = "Greys", aesthetics = "interval_color") +
...
p = preds |>
  ...
  geom_point(aes(y = mpg), data = cars, color = alpha(label_color, 0.65), stroke = 0.65, shape = 21, fill = alpha("white", 0.35), size = 1.25) +
...
p = two_a |>
  ...
  scale_x_continuous(breaks = NULL, limits = c(-3, 14)) +
  scale_y_discrete(expand = expansion(0.15)) +
  scale_fill_discrete_sequential(palette = "Grays", nmax = 8) +
  guides(fill = "none") +
  labs(x = NULL)
...
p = ggplot() +
  ...
  geom_vline(xintercept = 1, color = label_color, linewidth = 0.5) +
  label_(x = 1.75, y = 1, label = "median is here :)", vjust = 1) +
  scale_x_log10(limits = 10^c(-2,2), breaks = 10^(-2:2), labels = as.character) +
  scale_y_continuous(breaks = NULL) +
  labs(x = NULL, y = NULL)
