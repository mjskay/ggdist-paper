...
\vspace*{-17pt}
\caption{Flow of data through the visualization construction pipeline for a simple use of \texttt{ggdist::stat\_slab}. The \texttt{layer}, part of \textit{ggplot2}, applies initial aesthetic mappings, translating the input data into columns understood by \texttt{stat\_slab}, which then constructs a much larger data frame containing evaluations of distribution functions over an \textit{x} grid for each distribution (the user can specify the grid size if needed; the default size is 501). Then, \texttt{layer} applies a second set of aesthetic mappings, and \texttt{geom\_slab} uses the final data frame to construct the plot.}
\vspace*{-13pt}
...
In support of the above pipeline, \textit{ggdist} includes extensive utilities for manipulating distributions (some of which are exposed in its \text{api}), including functions for calculating various types of intervals and for determining various properties of distributions (e.g. detecting discrete or constant distributions, determining distribution limits). As the basic structure of the pipeline is governed by \textit{ggplot2}, my hope is that other similar grammar of graphics systems could easily implement a formalism like the one I've described, allowing them to support a variety of uncertainty visualizations through a distributional approach.
