...
Given this data frame, we the equivalent of the abstract specification above is as follows (adding a \textit{point} layer to show the original data too):
...
\textit{lineribbon} defaults to \texttt{.width = c(.5, .8, .95)} and maps the resulting \texttt{.width} onto the \texttt{fill} aesthetic, so we do not need to specify \texttt{.width} or the \texttt{fill} mapping for this example.
