...
\abstract{%
  ...
  A free copy of this paper is available at \href{https://osf.io/2gsz6}{https://osf.io/2gsz6}. All supplemental materials are available at \href{https://github.com/mjskay/ggdist-paper}{https://github.com/mjskay/ggdist-paper} and are archived on Zenodo at \href{https://doi.org/10.5281/zenodo.7770984}{doi:10.5281/zenodo.7770984}.
}
...
\teaser{
  ...
  \caption{%
   ...
   % ALT: A figure showing various examples of visualization types created by ggdist: (A) combinations of slabs and pointintervals showing univariate uncertainty depicted with CDFs or density functions overlaid with intervals; (B) lineribbons used to show uncertainty in a line plot by putting uncertainty bands of varying levels of probability mass around each line; (C) an anthropomorphic quantile dotplot of a Normal distribution with 50 dots, where each dot is an icon of a tiny person; (D) a logit dotplot of penguin body mass (x axis) against the probability a penguin is male, showing that heavier penguins are more likely to be male, using a lineribbon (showing the probability) combined with two dotplots showing each of the penguins in the dataset and their sex; (E) Raincloud plots, which combine a density plot of a sample, a dotplot showing each element of a sample, a 95% and 66% interval on each sample, and the median of each sample.
  }
  \label{fig:teaser}
}
...
%found works pretty well when I teach the grammar of graphics to undergraduates. I've used a variation on this notation 
...
% ALT: weight mapped onto x, mpg mapped onto y, and GEOM = point. This produces a scatterplot of weight (on the x axis) versus mpg (on the y axis) showing that heavier cars have lower mpg.
\equationfigure{
\begin{align*}
...
%, which gives an incorrect intuition in my view. 
...
%ALT: mu mapped onto x, sigma mapped onto x sub SD, GEOM = pointinterval. This produces an interval centered at mu with a width of plus or minus 2 times sigma.
\equationfigure{
\begin{align*}
\mu &\rightarrow x\\
...
% ALT: mu mapped onto x, mu minus 1.96 times sigma mapped onto x sub min, mu plus 1.96 times sigma mapped onto x sub max, and geom = pointinterval. This produces an interval with endpoints at mu plus or minus 1.96 times sigma, centered on mu.
\equationfigure{
\begin{align*}
\mu &\rightarrow x\\
...
\begin{itemize}
    ...
    \item   For a confidence distribution, $F_M(x) = \gamma$  is the confidence $\gamma$ at which $x$ would be the upper limit of a one-sided $\gamma\%$ confidence interval, $[-\infty, x]$, for $M$. %That sentence is pretty typical of convoluted frequentist definitions, so an example might illuminate: given the inverse \textsc{cdf}, $F_M^{-1}(\gamma)$, 
 %so it might be easier to think in terms of the inverse: $F_M^{-1}(\gamma)$ yields the value $x$, which is the upper limit of a one-sided $\gamma\%$ confidence interval on $M$: $[-\infty,x]$. So 
%$[-\infty, F_M^{-1}(0.95)]$ will be a one-sided 95\% confidence interval on $M$.
\end{itemize}
...
%ALT: median(M) mapped onto x, the quantile function of M at 1 minus gamma over 2 mapped onto x sub min, the quantile function of M at 1 plus gamma over 2 mapped onto x sub max, and geom = pointinterval. This produces an interval centered at the median of M with endpoints at the quantile function of M at 1 plus or minus gamma over 2.
\equationfigure{
\begin{align*}
...
%ALT: Same as the previous figure, but with gamma mapped onto linewidth and gamma taken from the set 0.66 and 0.95. This produces two intervals: a thinner 95% interval and a thicker 66% interval.
\equationfigure{
\begin{align*}
\operatorname{median}(M) &\rightarrow x\\
F_M^{-1}\left[\frac{1 - \gamma}{2}\right] &\rightarrow x_\textsc{min}\\
...
%ALT: same as the previous figure, but with gamma mapped onto color instead of linewidth, so that intervals with larger mass have a lighter color.
\equationfigure{
\begin{align*}
...
%ALT: M mapped onto x dist, stat = pointinterval, point = median, interval = quantile interval, and gamma in the set 0.66 and 0.95. This produces a point + interval plot with two intervals, a thinner 95% interval and a thicker 66% interval.
\equationfigure{
\begin{align*}
...
%ALT: Normal of mu sigma mapped onto x dist, stat = pointinterval, point = median, interval = quantile interval, and gamma in the set 0.66 and 0.95. This produces a point + interval plot with two intervals, like the previous example.
\equationfigure{
\begin{align*}
...
%ALT: t sub nu of mu sigma mapped onto x dist, stat = poininterval. This produces a similar two interval plot to the previous, but of a t distribution instead of a Normal distribution.
\equationfigure{
\begin{align*}
...
%ALT: log of mpg is drawn from Normal of g at weight and sigma
\[
...
% ALT: weight mapped onto x, p of mpg given weight mapped onto y dist, gamma mapped onto fill, stat = lineribbon, and gamma in the set 0.5, 0.8, 0.95. This produces a plot of weight versus mpg, with a curved line showing that higher weight predicts lower mpg, with 50, 80, and 95 percent uncertainty bands around the line.
\equationfigure{
\begin{align*}
\mathit{weight} &\rightarrow x\\
...
%ALT: Same as the previous figure, but with gamma being a set of 100 evenly-spaced values between 0 and 1 (exclusive). This produces a gradient-like line with uncertainty rather than three discrete bands.
\equationfigure{
\begin{align*}
\gamma &\rightarrow \mathit{fill}\\
\textsc{stat} &= \mathit{lineribbon}\\
...
%ALT: M mapped onto x dist, group mapped onto y, f sub M of x mapped onto thickness, and stat = slab. This produces two density, plots, one at the top of the y axis (corresponding to group b) and one at the bottom (corresponding to group a).
\equationfigure{
\begin{align*}
M &\rightarrow x_\textsc{dist}\\
...
%ALT: same as the previous, but with f sub M at x mapped onto alpha instead of thickness. This produces density plots as before, but instead of using height to show density, opacity depicts density.
\equationfigure{
\begin{align*}
M &\rightarrow x_\textsc{dist}\\
\textit{group} &\rightarrow y\\
...
%ALT: negative max of either the absolute value of two times the CDF of M at x minus 1 or 0.95 mapped onto alpha. This again produces color gradient plots, but the color is solid between the lower and upper bounds of the 95 percent interval, and fades out into the tails.
\equationfigure{
\begin{align*}
...
% ALT: A demonstration of a consonance curve, which peaks at the medien of the distribution. If you draw a horizontal line at 0.5 on a consonance curve, it intersects in two places which form a 50 percent interval on the distribution. If you draw a horizontal line at 0.05, it intersects at the 95 percent interval.
\equationfigure{
\begin{align*}
...
%ALT: Same as the Correll and Gleicher example, but with f sub M at x also mapped onto thickness. The result is that the shapes of the slabs also taper to a point (in addition to fading out).
\equationfigure{
\begin{align*}
f_M(x) &\rightarrow \textit{thickness}\\
...
%ALT: Same as the previous example, but now gamma is mapped onto fill and gamma is taken from the set 0.5, 0.8, 9.5, and 1. This produces a similar visualization, except instead of color fading out in a smooth gradient, it is discretized into values corresponding to each of the intervals defined by gamma.
\equationfigure{
\begin{align*}
f_M(x) &\rightarrow \textit{thickness}\\
...
%ALT: if gamma is less than or equal to 0.95, log of f sub M at x is mapped onto thickness. This produces eyeball-like shapes that are drawn only inside the 95 percent interval, instead of the usual mirrored bell-curve shape that smoothly tapers out towards infinity.
\equationfigure{
\begin{align*}
...
%ALT: 1 minus the CDF of M at x mapped onto alpha, stat = slab. This produces a plot that looks like a bar chart where the tops of the bars fade out.
\equationfigure{
\begin{align*}
...
%ALT: f of M at x mapped onto thickness, absolute value of x less than 2 mapped onto fill, and stat = slab. This produces densities plots where the area under each density curve that is between -2 and 2 is highlighted in blue.
\equationfigure{
\begin{align*}
f_M(x) &\rightarrow \textit{thickness}\\
...
%ALT: M mapped onto x dist, group mapped onto y, stat = dots, and quantiles = 100. This produces discretized density plots (quantile dotplots) where each area is made up of 100 dots.
\equationfigure{
\begin{align*}
M &\rightarrow x_\textsc{dist}\\
\textit{group} &\rightarrow y\\
...
%ALT: M mapped onto x dist, x greater than 0 mapped onto shape, stat = dots, and quantiles = 50. This produces a quantile dotplot of a normal distribution with 50 dots centered on 0, where dots above 0 are smiley faces and dots below 0 are frowney faces.
\equationfigure{
\begin{align*}
M &\rightarrow x_\textsc{dist}\\
...
This plot does require us to specify what the \textit{shape} scale function is. Abstractly, if we assume each aesthetic mapping \textit{foo} has a scale function $s_\textit{foo}$, we might define the scale function for \textit{shape} as:

%ALT: the function s sub shape at x is defined as x mapped onto a smiley face if x is true and a frowney face if x is false.
...
%ALT: In step A, layer applies the before-stat mappings t sub nu at mu sigma mapped onto x dist and group mapped onto y. In step B, stat_slab creates an x grid and at each x calculates f(x), F(x), and the mass (gamma) of the smallest requested interval containing at x. At step C, the layer applies after-stat mappings: f(x) mapped onto thickness and gamma mapped onto fill. At step D, geom_slab constructs multiple polygons for each slab, one for each block of consecutive x values with the same appearance (e.g. the same fill).
\begin{figure*}[t!]
\vspace*{-10pt}
...
%noticed users struggle less with the new design: previously, the $\textit{dist}$ aesthetic would implicitly be mapped to the relevant positional scale depending on the orientation of the geometry, and this led to confusion when this mapping was not as a user intended. Since the change, I have 
...
%Here I'll outline the data processing pipeline that makes some of what \textit{ggdist} does possible; in particular, I'll describe how the slab statistic and geometry work. [TBD insert forward/backward footnote ref re: distinction between stat and geom]
...
%as of this writing is one of the only R packages to do so (even base \textit{ggplot2} has not implemented it yet).

%The \texttt{segments} algorithm (depicted in the figure) splits 


%; to do this, it must determine a reasona

%, and \texttt{geom\_slab}, which constructs the low-level graphical objects used to display the slab. 



%The interested implementer might refer to 

%Add bit about gradient rendering and reference Paul Murrell's article
...
%\footnote{Pedants get one properly typeset ``formul\ae'' per paper. There it was.} 
...
%Users often request shortcuts for creating specific chart types to solve a problem at hand, which is a not-unreasonable motivation. 
...
%ALT: f sub Y at y equals f sub X at the inverse of g at y, times the absolute value of the derivative of the inverse of g at y.
\[
...
%ALT: ggplot() + stat_function(fun = \(x) dlnorm(x, 0, 1)) produces a typical density plot of a log-normal distribution.
\noindent
\begin{minipage}{.5\columnwidth}
\small
\begin{verbatim}
...
%ALT: ggplot() + stat_function(fun = \(x) dlnorm(x, 0, 1)) + scale_x_log10() does not produce a correct density plot of a log-normal distribution on a log scale; the distribution is not centered on 1 as it should be.
\noindent
\begin{minipage}{.5\columnwidth}
\small
\begin{verbatim}
...
%ALT: ggplot() + aes(xdist = dist_lognormal(0,1)) + stat_slab() + scale_x_log10() produces a correct plot of a log-normal distribution on a log scale.
\noindent
\begin{minipage}{.5\columnwidth}
\small
\begin{verbatim}
...
%\footnote{Let's be honest, Xiaoying did all the hard work.} 
