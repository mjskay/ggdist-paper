...
\vspace*{-17pt}
\caption{Flow of data through the visualization construction pipeline for a simple use of \texttt{ggdist::stat\_slab}. The \texttt{layer}, part of \textit{ggplot2}, applies initial aesthetic mappings, translating the input data into columns understood by \texttt{stat\_slab}, which then constructs a much larger data frame containing evaluations of distribution functions over a grid for each distribution. \texttt{layer} applies a second set of aesthetic mappings, then \texttt{geom\_slab} uses the final data frame to construct the plot.}
\vspace*{-13pt}
