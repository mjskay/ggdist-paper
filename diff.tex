...
%DIF 114-115c114-115
%DIF < \setlength{\parskip}{.5\belowdisplayskip}%
%DIF < \vspace{.65\abovedisplayskip}%
%DIF -------
\setlength{\parskip}{.4\belowdisplayskip}% %DIF > 
\vspace{.55\abovedisplayskip}% %DIF > 
%DIF -------
\noindent%
\small%
\centering%
...
%DIF 143-144c143-144
%DIF < \setlength{\abovedisplayskip}{0}
%DIF < \setlength{\belowdisplayskip}{0}
%DIF -------
\setlength{\abovedisplayskip}{0pt} %DIF > 
\setlength{\belowdisplayskip}{0pt} %DIF > 
%DIF -------
#1\end{minipage}%
...
\textit{ggdist}\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{kay2023ggdist} }\hskip0pt%DIFAUXCMD
}\DIFaddend is an attempt to rectify this situation. It started under the guise of \textit{tidybayes}\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{kay2023tidybayes}}\hskip0pt%DIFAUXCMD
}\DIFaddend ---an R package I wrote for post-processing Bayesian models for use with \textit{ggplot2}---in \DIFdelbegin \DIFdel{about }\DIFdelend 2016. \textit{tidybayes} slowly gained \DIFdelbegin \DIFdel{some use }\DIFdelend \DIFaddbegin \DIFadd{usage }\DIFaddend in the Bayesian statistics community, but the package always had two complementary, but not perfectly aligned, \DIFdelbegin \DIFdel{use cases}\DIFdelend \DIFaddbegin \DIFadd{uses}\DIFaddend : post-processing Bayesian model output for visualization, and creating uncertainty visualizations in the grammar of graphics. The latter is bigger than \textit{just} Bayesian statistics: everyone needs to visualize uncertainty! \DIFdelbegin \footnote{\DIFdel{And, contrary to popular opinion---as we'll see later---Bayesian and frequentist uncertainty visualization can in fact be done within the same framework.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{And, contrary to popular opinion---as we'll see later---Bayesian and frequentist uncertainty visualization can be done within the same framework. }\DIFaddend Recognizing this broader need, in 2020 I spun off the uncertainty visualization components of \textit{tidybayes} into a new package, \textit{ggdist}, and published it to \textsc{cran}\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{hornik2012cran}}\hskip0pt%DIFAUXCMD
}\DIFaddend . Since then it has steadily grown in use inside and outside the Bayesian statistics community in R, and now averages about 14,000 downloads per month (best described as ``modest''\DIFdelbegin \DIFdel{for an R package). }\DIFdelend \DIFaddbegin \DIFadd{). %DIF >  for an R package).
}\DIFaddend 

\textit{ggdist} aims to be (1) a coherent extension to the grammar of graphics that makes it easy to create a variety of uncertainty visualizations, and (2) an implementation of a particular formalism for describing uncertainty visualizations: mappings of functions of distributions (e.g. densities, \textsc{cdf}s, quantiles) onto aesthetics (e.g. height, color, opacity). \DIFaddbegin \DIFadd{That formalism is the primary focus and contribution of this paper. }\DIFaddend My ambition is that not only should that formalism be able to express a variety of uncertainty visualizations through a single coherent framework, but that that framework should be complete enough that someone else can wander along and use it to express new uncertainty visualization types I've never thought of before.\footnote{Emphasis on \textit{I}---I think a formalism and its corresponding \textsc{api} truly shows its power when people make things with it that its creator did not conceive.} Naturally this is hard to demonstrate, but I will attempt to do so by giving a tour of \textit{ggdist} through recreations of a variety of uncertainty visualizations in the literature. Along the way, I'll discuss lessons I've learned in how to effectively integrate uncertainty visualization into the grammar of graphics.
...
Given the huge expressive power of the grammar of graphics and the popularity of tools built on it, I hope a \DIFdelbegin \DIFdel{catalog of my experience }\DIFdelend \DIFaddbegin \DIFadd{systematic approach to integrating uncertainty into the grammar of graphics }\DIFaddend might provide a catalyst for improved implementations of uncertainty visualization to flourish in existing grammar of graphics ecosystems, and ultimately for even better formal descriptions of uncertainty visualization to arise. 
\DIFdelbegin \DIFdel{Let's give it a try.
}\DIFdelend %DIF > Let's give it a try.

\section{Setting the stage}

\subsection{A simplified notation for the grammar of graphics}

\DIFdelbegin \DIFdel{To be able }\DIFdelend \DIFaddbegin \DIFadd{As I would like }\DIFaddend to talk more generally than a specific grammar of graphics \DIFdelbegin \DIFdel{implementation, we'll at least }\DIFdelend \DIFaddbegin \DIFadd{implementation---that is, I am more concerned with the formalism underlying }\textit{\DIFadd{ggdist}} \DIFadd{than with the idiosyncrasies of its }\textsc{\DIFadd{api}}\DIFadd{---I'll }\DIFaddend need a formal way of writing down visualization specifications separated from a particular implementation. I'll adopt here a notation that I've \DIFdelbegin \DIFdel{found works pretty well when I teach the grammar of graphics to undergraduates. I've used
a variation on this notation }\DIFdelend \DIFaddbegin \DIFadd{used
%DIF > found works pretty well when I teach the grammar of graphics to undergraduates. I've used a variation on this notation 
}\DIFaddend when teaching \textit{ggplot2}, \textit{Vega-lite}, \textit{Altair}, and \textit{Tableau}\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{. I've }\DIFaddend found students pick it up handily, which is at least some evidence for its understandability. The core notation describes a visualization in terms of its \textit{data variables}, \textit{aesthetic mappings}, and \textit{geometries}. We create a scatterplot of the infamous \texttt{mtcars} dataset~\cite{henderson1981building}, for example, by mapping one variable onto the \textit{x} aesthetic and another onto the \textit{y} aesthetic:\footnote{I am aware it is traditional to slap a number on each figure and float it off into a random corner of the page. I violate that norm throughout this paper without remorse.}
...
%DIF > , which gives an incorrect intuition in my view. 
...
%DIFAUXCMD
\DIFdelend \DIFaddbegin \vspace{5pt}
\noindent
\begin{minipage}{.4\columnwidth}%DIF > 
\textbf{\DIFadd{~~ggplot2}}\DIFadd{:
}\DIFaddend 
...
\end{verbatim}%DIF > 
\end{minipage}%DIF > 
\begin{minipage}{.5\columnwidth}%DIF > 
\textbf{\DIFadd{~~Vega-lite}}\DIFadd{:
}
...
\DIFaddend This shows the close correspondence between the abstract notation above and the particulars of code in actual grammar of graphics implementations. Throughout the rest of this paper, I'll stick to the abstract notation and corresponding \textit{ggplot2} + \textit{ggdist} code for some particulars.
...
}\DIFdelend %DIF > That sentence is pretty typical of convoluted frequentist definitions, so an example might illuminate: given the inverse \textsc{cdf}, $F_M^{-1}(\gamma)$, 
 %DIF > so it might be easier to think in terms of the inverse: $F_M^{-1}(\gamma)$ yields the value $x$, which is the upper limit of a one-sided $\gamma\%$ confidence interval on $M$: $[-\infty,x]$. So 
%DIF > $[-\infty, F_M^{-1}(0.95)]$ will be a one-sided 95\% confidence interval on $M$.
\end{itemize}
...
\DIFdelbegin \footnote{\DIFdel{Students of the grammar of graphics will note that I've left off details of the scale functions here (and elsewhere). In this case, the $linewidth$ mapping and scale function will need to include conversion to an ordinal data type (}\texttt{\DIFdel{ordered}} %DIFAUXCMD
\DIFdel{in R). This is mostly too minor a concern for the high level at which I want to keep notational discussion in this paper, though there is an example in }%DIFDELCMD < \cref{sec:dotplots} %%%
\DIFdel{where we'll have to get into it.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend %DIF > \footnote{Students of the grammar of graphics will note that I've left off details of the scale functions here (and elsewhere). In this case, the $linewidth$ mapping and scale function will need to include conversion to an ordinal data type (\texttt{ordered} in R) with a reversed order (smaller $\gamma$ means thicker \textit{linewidth}). This is mostly too minor a concern for the high level at which I want to keep notational discussion in this paper, though there is an example in \cref{sec:dotplots} where we'll have to get into it.} 
...
On the other hand, these are still a bit low-level: they require the user to know how to calculate interval endpoints from the quantile function. This also limits us specifically to quantile intervals, when other intervals types, such as highest-density intervals~\cite{hyndman1996computing} \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend shortest intervals~\cite{liu2015spi}, might be preferable. Thus, \textit{ggdist} also supplies a \textit{stat} version of \textit{pointinterval}, which bundles up some statistical calculations and default aesthetic mappings with the \textit{pointinterval} geometry.\DIFaddbegin \footnote{\DIFadd{See }\cref{sec:implementation}\DIFadd{, or Wickham~\mbox{%DIFAUXCMD
\cite{wickham2010layered}}\hskip0pt%DIFAUXCMD
, for more on }\textit{\DIFadd{stat}}\DIFadd{s and }\textit{\DIFadd{geom}}\DIFadd{s.}} \DIFaddend All \textit{stat}s in \textit{ggdist} support the $x_\textsc{dist}$ and $y_\textsc{dist}$ aesthetics, onto which objects that represent distributions can be mapped. They also allow the user to specify the type of point and interval used, and generate the corresponding values and mappings for $x$, $x_\textsc{min}$, $x_\textsc{max}$, and \textit{linewidth}. This changes the specification to something like:
...
\DIFdelbegin \DIFdel{Which in }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend \textit{ggdist} \DIFdelbegin \DIFdel{is:
}%DIFDELCMD < 

%DIFDELCMD < \begin{centerverbatim}
%DIFDELCMD < %%%
\noindent\DIFdel{\quad\texttt{ggplot(data) +}}

\noindent\DIFdel{\qquad\texttt{aes(xdist = dist\_student\_t(nu, mu, sigma)) +}}

\noindent\DIFdel{\qquad\texttt{stat\_pointinterval()}}

%DIFDELCMD < \end{centerverbatim}
%DIFDELCMD < %%%
\noindent\DIFdel{Thus the code matches up closely to the abstract notation}\DIFdelend \DIFaddbegin \DIFadd{code, the aesthetic specification closely matches this abstract notation: }{\color{blue}%DIFAUXCMD
\verb|aes(xdist = dist_student_t(nu, mu, sigma))|%
}%DIFAUXCMD
\DIFaddend .

%DIF > \begin{centerverbatim}
%DIF > ggplot(data) +
%DIF >   aes(xdist = dist_student_t(nu, mu, sigma)) +
%DIF >   stat_pointinterval()
%DIF > \end{centerverbatim}
%DIF > Thus the code matches up closely to the abstract notation.
\DIFaddbegin 

\DIFaddend \subsection{Ribbons}
...
I \DIFdelbegin \DIFdel{include }\DIFdelend \DIFaddbegin \DIFadd{also show }\DIFaddend the raw data as a separate \textit{point} \DIFdelbegin \DIFdel{geometry layeras well}\DIFdelend \DIFaddbegin \DIFadd{layer}\DIFaddend , for comparison. Assuming \texttt{fit}  is a Bayesian version of \DIFdelbegin \DIFdel{such a }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend model fit using the \textit{brms} modeling package\DIFdelbegin \DIFdel{in R}\DIFdelend , and \texttt{preds} is a data frame of \DIFdelbegin \DIFdel{desired }\DIFdelend \textit{weight} values to predict on, we can add a column to \texttt{preds} that contains a random variable representation of $p(\mathit{mpg} \mid \mathit{weight})$ using \texttt{brms::posterior\_predict}~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{burkner2018brms} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{burkner2017brms} }\hskip0pt%DIFAUXCMD
}\DIFaddend and the \texttt{posterior::rvar}  data type~\cite{burkner2022posterior}. The latter is a data type I created \DIFdelbegin \footnote{\DIFdel{This was a slightly insane idea in itself, given the way that base R data types work. As R's classes work based on generic }\textit{\DIFdel{functions}}%DIFAUXCMD
\DIFdel{, rather than classes with a well-defined set of methods, this requires tracking down all the various functions implemented to work with arrays and vectors in the language and providing implementations for them for random variable arrays. Indexing functions especially are annoying, as there are half a dozen different ways to index into R arrays, each with myriad corner cases. Anyway.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend specifically to wrap large samples that represent distributions into objects that mimic R vectors and arrays,
%DIF > \footnote{This was a slightly insane idea in itself, given the way that base R data types work. As R's classes work based on generic \textit{functions}, rather than classes with a well-defined set of methods, this requires tracking down all the various functions implemented to work with arrays and vectors in the language and providing implementations for them for random variable arrays. Indexing functions are especially annoying, as there are half a dozen different ways to index into R arrays, each with myriad corner cases. Anyway.}
...
\begin{enumerate}
    ...
    }\DIFaddend \item \texttt{aes(thickness = !!p\_(x))}:  This uses a small domain-specific language for probability expressions I added to \textit{ggdist}, and is intended to more closely mimic a mapping like $p(x) \rightarrow thickness$ in code. The \texttt{!!} pseudo-operator comes from the \textit{rlang} meta-programming R package, and performs \textit{unquotation}~\cite{wickham2019advanced}: it inserts the expression returned by \texttt{p\_(x)}, which in this case is \texttt{after\_stat(pdf)}, into the \texttt{aes} call.\footnote{\DIFaddbegin \label{foot:after-stat} \DIFaddend I added this syntax \DIFdelbegin \DIFdel{relatively recently, }\DIFdelend when \textit{ggplot2} deprecated \texttt{stat()}, which \DIFdelbegin \DIFdel{used to be }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend a synonym for \texttt{after\_stat()}. This (1) made expressions with \texttt{after\_stat} needlessly verbose and (2) replaced a declarative verb, \texttt{stat}---which indicates the type of expression \DIFdelbegin \DIFdel{at play }\DIFdelend but not when it is computed---with a procedural verb, \texttt{after\_stat}. I feel that this \DIFdelbegin \DIFdel{name }\DIFdelend change violates the declarative foundations of \textit{ggplot2}, and the mini-\textsc{dsl} for probabilistic expressions \DIFdelbegin \DIFdel{in }\textit{\DIFdel{ggdist}} %DIFAUXCMD
\DIFdelend is my small protest against it.}
\end{enumerate}

Slab geometries have several useful properties \DIFdelbegin \DIFdel{which help make them useful for creating }\DIFdelend \DIFaddbegin \DIFadd{designed to make it easy to create }\DIFaddend a variety of uncertainty visualizations. One important property is that the \textit{alpha} (opacity), \textit{fill}, and outline \textit{color} aesthetics of the slabs can have data values mapped onto them at a sub-geometry level.\footnote{Just as an implementation note, this happens to be an incredible pain in the ass. \DIFdelbegin \DIFdel{Prior to R 4.1, }\DIFdelend \DIFaddbegin \DIFadd{See }\DIFaddend the \DIFdelbegin \DIFdel{R graphics engine did not have proper gradient support, so this involved manually sub-dividing geometries and interpolating thickness values at cutpoints between contiguous blocks }\DIFdelend \DIFaddbegin \DIFadd{description }\DIFaddend of \DIFdelbegin \DIFdel{color. Now, with proper gradient support }\DIFdelend \DIFaddbegin \texttt{\DIFadd{geom\_slab}} \DIFadd{rendering }\DIFaddend in \DIFdelbegin \DIFdel{some R graphics output formats (e.g. }\textsc{\DIFdel{svg}} %DIFAUXCMD
\DIFdel{and }\textsc{\DIFdel{pdf}}%DIFAUXCMD
\DIFdel{), }\textit{\DIFdel{ggdist}} %DIFAUXCMD
\DIFdel{can output high-quality color gradients, and as of this writing is one of the only R packages to do so (even base }\textit{\DIFdel{ggplot2}} %DIFAUXCMD
\DIFdel{has not implemented it yet)}\DIFdelend \DIFaddbegin \cref{sec:implementation}\DIFaddend .} This functionality, combined with the ability to map arbitrary distribution functions, means we can easily recreate a bunch of visualizations from the literature. The obvious first example would be a color gradient plot~\cite{jackson2008displaying}, by mapping density onto \textit{alpha}:
...
Helske describes these violin-interval plots as ``more challenging to create''; \textit{ggdist} supports them naturally through a combination of its core features\DIFdelbegin \DIFdel{. This demonstrates the power of }\DIFdelend \DIFaddbegin \DIFadd{: making interval mass ($\gamma$) into data and }\DIFaddend allowing distribution functions and intervals to be \DIFdelbegin \DIFdel{easily }\DIFdelend mapped to aesthetics within a single \DIFdelbegin \DIFdel{geometry}\DIFdelend \DIFaddbegin \DIFadd{slab}\DIFaddend .
...
I would describe this chart type as theoretically interesting but interpretationally problematic. Anecdotally, without the overlaid reference point and interval, folks I show this to often do not correctly identify the mean\DIFdelbegin \DIFdel{of the distribution}\DIFdelend , believing the darker parts of the bar are more likely (a \DIFdelbegin \DIFdel{not unreasonable }\DIFdelend \DIFaddbegin \DIFadd{not-unreasonable }\DIFaddend misinterpretation).\footnote{I don't know if this is an example of within-the-bar bias~\cite{newman2012bar} or not.} On the other hand, I do like the idea of \textit{generative stories} in uncertainty visualization; \DIFdelbegin \DIFdel{for example}\DIFdelend \DIFaddbegin \DIFadd{e.g.}\DIFaddend , I've used Plinko boards to \href{https://presidential-plinko.com/}{depict uncertainty in election forecasts}, which try to capitalize on a physical process---one that might intuitively\footnote{As this is an uncertainty visualization paper, I am professionally obligated to use ``intuitive'' at least once without defining it.} feel random to people---to depict uncertainty through a form of \textit{sedimentation}~\cite{huron2013sediment}.

Another advantage to being able to map arbitrary data values onto slab aesthetics is that we can map the results of logical conditions onto those aesthetics. A common procedure in \DIFdelbegin \DIFdel{the Bayesian estimation community }\DIFdelend \DIFaddbegin \DIFadd{Bayesian estimation }\DIFaddend is to use regions of practical equivalence (\textsc{rope}s)~\cite{kruschke2018rejecting}. Say \DIFdelbegin \DIFdel{, for example, }\DIFdelend you are interested in whether an estimate is ``practically'' equal to \DIFdelbegin \DIFdel{0; you would }\DIFdelend \DIFaddbegin \DIFadd{0. You could }\DIFaddend define a \textsc{rope} \DIFdelbegin \DIFdel{around 0 of $\pm$ }\DIFdelend \DIFaddbegin \DIFadd{of $0\pm$ }\DIFaddend some small effect size \DIFdelbegin \DIFdel{within which you would consider the value }\DIFdelend \DIFaddbegin \DIFadd{that you consider }\DIFaddend so close to 0 it is effectively indistinguishable from it. Then you can ask: \textit{what is the probability the value is \DIFaddbegin \DIFadd{in the }\textsc{\DIFadd{rope}}\DIFadd{---i.e., }\DIFaddend practically equivalent to 0?} We can visualize this probability by highlighting the \textsc{rope} on a density:
...
\DIFdelbegin \DIFdel{Here, }\DIFdelend $\Pr(\lvert x \rvert < 2)$---the probability the value is in a \textsc{rope} of $\pm2$---is directly encoded by the proportion of the \DIFdelbegin \DIFdel{area under the curve }\DIFdelend \DIFaddbegin \DIFadd{slab }\DIFaddend highlighted in blue. \DIFdelbegin \DIFdel{In }\textit{\DIFdel{ggplot2}}%DIFAUXCMD
\DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{Because }\DIFaddend logical conditions can be mapped directly onto aesthetics, \DIFdelbegin \DIFdel{so in }\textit{\DIFdel{ggdist}}%DIFAUXCMD
\DIFdel{, }\DIFdelend we can use \texttt{aes(fill = after\_stat(abs(x) < 2))} to create \DIFdelbegin \DIFdel{the above }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend chart.

\subsection{Dotplots}
\label{sec:dotplots}

These days, when every other uncertainty visualization paper is about \textit{frequency framing} or \textit{discrete outcome} \DIFdelbegin \DIFdel{representations---spaghetti }\DIFdelend \DIFaddbegin \DIFadd{displays---spaghetti }\DIFaddend plots~\cite{cox2013visualizing,liu2018visualizing} hypothetical outcome plots~\cite{hullman2015hypothetical,kale2018hypothetical}, quantile dotplots~\cite{kay2016ish,fernandes2018uncertainty} and so on---I would be remiss to write a whole uncertainty visualization toolkit that can't make one measly discrete outcome visualization.\footnote{Plus \DIFdelbegin \DIFdel{, }\DIFdelend it would be a \DIFdelbegin \DIFdel{little }\DIFdelend \DIFaddbegin \DIFadd{bit }\DIFaddend silly \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend me to develop a whole-ass visualization toolkit that can't at least make quantile dotplots, since I came up with them in the first place~\cite{kay2016ish}.} The natural counterpart to the continuous encoding of the \textit{slab} geometry is \textit{ggdist}'s \textit{dots} geometry, which \DIFdelbegin \DIFdel{is designed to }\DIFdelend \DIFaddbegin \DIFadd{can }\DIFaddend create Wilkinson dotplots~\cite{wilkinson1999dot} (\DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend raw data)\footnote{And minor variations thereof, like \textit{beeswarm} plots, which can be created by setting $\textsc{side} = \textit{both}$ and (optionally) adjusting the \textsc{layout} parameter; see the \href{https://mjskay.github.io/ggdist/articles/dotsinterval.html}{\textit{dotsinterval} vignette} for examples.} and quantile dotplots \DIFaddbegin \DIFadd{(to show uncertainty)}\DIFaddend .
...
This plot does require \DIFdelbegin \DIFdel{specifying }\DIFdelend \DIFaddbegin \DIFadd{us to specify }\DIFaddend what the \textit{shape} scale function is. Abstractly, if we assume each aesthetic mapping \textit{foo} has a scale function $s_\textit{foo}$, we might define the scale function for \textit{shape} as:
...
\DIFdelbegin %
{\color{red}%DIFAUXCMD
\verb|scale_shape_manual(|%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \verb|scale_shape_manual(|\\
%
{\color{red}%DIFAUXCMD
\verb|  values = c("TRUE" = "|%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \verb|  values = c("TRUE" = "|%%%
...
%DIF >   
\DIFaddend \endvarwidth%DIF > 
\par\DIFdelbegin %DIFDELCMD < \vspace{.5\belowdisplayskip}
%DIFDELCMD < %%%
\DIFdelend %DIF > 
\DIFaddbegin \vspace{.1\belowdisplayskip}%DIF > 
\DIFaddend \hfill\break%DIF > 
}
...
My hope in the preceding sections was to whet your appetite for the \DIFdelbegin \DIFdel{possibilities }\DIFdelend \DIFaddbegin \DIFadd{expressive power }\DIFaddend of an uncertainty visualization grammar grounded in distributions, both of probability and of confidence. \DIFdelbegin \DIFdel{I have attempted }\DIFdelend \DIFaddbegin \DIFadd{It was not intended as a tutorial (hence the paucity of code), but }\DIFaddend to give a taste for \DIFdelbegin \DIFdel{why ggdist is structured the way it is, its underlying formalism , }\DIFdelend \DIFaddbegin \DIFadd{the underlying formalism of }\textit{\DIFadd{ggdist}}\DIFadd{, how it can help us think about uncertainty visualizations, }\DIFaddend and how it extends the grammar of graphics in a principled way to support fluid specification of a variety of uncertainty visualizations. \DIFdelbegin \DIFdel{What I've shown only scratches the surface of what }\textit{\DIFdel{ggdist}} %DIFAUXCMD
\DIFdel{can do: the interested reader 
(hopefully you!) }\DIFdelend \DIFaddbegin \DIFadd{For more examples, and specifically for examples with code, the interested reader 
%DIF > (hopefully you!) 
}\DIFaddend can check out the vignettes in the \href{https://mjskay.github.io/ggdist/}{\textit{ggdist} documentation}, which include a slew of examples not covered here\DIFdelbegin \DIFdel{. The }\DIFdelend \DIFaddbegin \DIFadd{, and the vignettes in the }\DIFaddend \href{https://mjskay.github.io/tidybayes/}{\textit{tidybayes} documentation}\DIFdelbegin \DIFdel{is also worth checking out, as it includes more }\DIFdelend \DIFaddbegin \DIFadd{, which include }\DIFaddend examples of the use of \textit{ggdist} with Bayesian models.
...
\textit{ggdist} currently sees about 14,000 downloads per month from \textsc{cran}~\cite{csardi2019cranlogs} (up from just a few thousand on original launch in 2020, piggy-backing on \textit{tidybayes}' already-established community\DIFdelbegin \DIFdel{at the time}\DIFdelend ). Per Google Scholar, it has been cited 46 times (and \textit{tidybayes} 236 times).\footnote{These are all citations of the software itself: neither package has had a paper written about it---yet.} Its Github project has been starred \textasciitilde 620 times, putting it in the company of popular packages like \textit{cowplot} (\textasciitilde 640), \textit{ggalt} (\textasciitilde 630), and \textit{ggtext} (\textasciitilde 600) in the \href{https://exts.ggplot2.tidyverse.org/gallery/}{\textit{ggplot2} extension gallery}. \DIFdelbegin \DIFdel{On Github, }\textit{\DIFdel{ggdist}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{It }\DIFaddend has 169 issues in its \DIFaddbegin \DIFadd{Github }\DIFaddend issue tracker, not counting the 305 issues in \textit{tidybayes} (\DIFdelbegin \DIFdel{of which many issues prior to }\DIFdelend \DIFaddbegin \DIFadd{many of which are }\DIFaddend \textit{ggdist}\DIFdelbegin \DIFdel{splitting from }\textit{\DIFdel{tidybayes }}%DIFAUXCMD
\DIFdel{were }\textit{\DIFdel{ggdist}}%DIFAUXCMD
\DIFdelend -related \DIFaddbegin \DIFadd{issues from before }\textit{\DIFadd{ggdist}} \DIFadd{and }\textit{\DIFadd{tidybayes}} \DIFadd{split}\DIFaddend ). Issues especially can be a sign of user engagement, because projects without broader engagement will have issues opened only by the author. To better understand that engagement, I exported all issues from \textit{ggdist} (see \href{https://github.com/mjskay/ggdist/issues}{Github} or the supplement) and read back through them, engaging in a light tagging process based on what I recalled about those issues.
...
\vspace*{-17pt}
\caption{\DIFaddFL{Flow of data through the visualization construction pipeline for Helske~\mbox{%DIFAUXCMD
\cite{helske2021can}}\hskip0pt%DIFAUXCMD
-style interval+density plot using }\texttt{\DIFaddFL{ggdist::stat\_slab}}\DIFaddFL{. The }\texttt{\DIFaddFL{layer}}\DIFaddFL{, part of }\textit{\DIFaddFL{ggplot2}}\DIFaddFL{, applies initial aesthetic mappings, translating input data into columns understood by }\texttt{\DIFaddFL{stat\_slab}}\DIFaddFL{, which then constructs a larger data frame containing evaluations of distribution functions over an }\textit{\DIFaddFL{x}} \DIFaddFL{grid for each distribution (the user can specify the grid size if needed; the default size is 501). Then, }\texttt{\DIFaddFL{layer}} \DIFaddFL{applies a second set of aesthetic mappings, and }\texttt{\DIFaddFL{geom\_slab}} \DIFaddFL{uses the final data frame to construct the plot.}}
\vspace*{-13pt}
\label{fig:implementation}
\end{figure*}


\DIFaddend Of the 169 issues on Github, 79 (47\%) were opened by someone other than me; there were 45 unique authors, excluding me. A further 16 issues were opened by me in direct response to some user need, either flowing from a comment on another issue or from a conversation \DIFdelbegin \DIFdel{I had with a user }\DIFdelend on another platform, often Twitter.\footnote{I spend possibly too much time on \texttt{\#rstats} Twitter engaging with data scientists around visualization problems.} Thus, just over half of all issues on \textit{ggdist}'s tracker stem from user \DIFdelbegin \DIFdel{engagement with the package---and }\DIFdelend \DIFaddbegin \DIFadd{engagement---and }\DIFaddend from a variety of users. I roughly categorized \DIFdelbegin \DIFdel{the types of }\DIFdelend issues (with some overlap) as follows:

\begin{itemize}
\DIFaddbegin \setlength{\parskip}{0pt}
    \DIFaddend \item 38\%: A request that did (or would) result in a new feature.
    \item 20\%: A user asking for help, usually with a particular plot they are trying to create.
    \item 17\%: A bug.
    \item 16\%: Internal issues, such as code refactoring, cleanup, or \textsc{todo}\DIFdelbegin \DIFdel{-tracking}\DIFdelend \DIFaddbegin \DIFadd{s}\DIFaddend .
...
\DIFdelbegin \footnote{\DIFdel{But who has the time...}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend %DIF > \footnote{But who has the time...} 
...
\section{\DIFdelbegin \DIFdel{Reflections and lessons learned}\DIFdelend \DIFaddbegin \DIFadd{How does all this work, anyway?}\DIFaddend }
\DIFaddbegin \label{sec:implementation}
\DIFaddend 

\DIFaddbegin \DIFadd{Interested implementers might ask: }\textit{\DIFadd{whither the implementation details?}} \DIFadd{The glib response, of course, is the }\DIFaddend \textit{ggdist} \DIFdelbegin \DIFdel{has been under development in some form or another for about six--seven years, and over that time has evolved through user feedback and several waves of refactoring and rewriting. Through all that,  I hope }\DIFdelend \DIFaddbegin \DIFadd{source code, which is well-tested (with near-100\% test coverage), available }\href{https://github.com/mjskay/ggdist}{\DIFadd{on Github}}\DIFadd{, and archived }\href{https://doi.org/10.5281/zenodo.3879620}{\DIFadd{on Zenodo}}\DIFadd{. The less glib response is: this section of the paper. 
}

%DIF > Here I'll outline the data processing pipeline that makes some of what \textit{ggdist} does possible; in particular, I'll describe how the slab statistic and geometry work. [TBD insert forward/backward footnote ref re: distinction between stat and geom]

\DIFadd{Space being artificially scarce,}\footnote{\DIFadd{Perhaps one day we'll throw off the shackles of the }\textsc{\DIFadd{ieee}}\DIFadd{.}} \DIFadd{I'll outline the data processing pipeline (}\cref{fig:implementation}\DIFadd{) for just the }\textit{\DIFadd{slab}} \DIFadd{statistic/geometry; the others follow similar principles. The basic implementation of slabs relies on two entities: }\texttt{\DIFadd{stat\_slab}} \DIFadd{and }\texttt{\DIFadd{geom\_slab}}\DIFadd{. Both of these objects are contained inside a }\textit{\DIFadd{ggplot2}} \texttt{\DIFadd{layer}}\DIFadd{, which amongst other things is responsible for applying aesthetic mappings and scale functions. For more details on how }\textit{\DIFadd{stat}}\DIFadd{s, }\textit{\DIFadd{geom}}\DIFadd{s, and }\textit{\DIFadd{layer}}\DIFadd{s work in }\textit{\DIFadd{ggplot2}}\DIFadd{, see Wickham~\mbox{%DIFAUXCMD
\cite{wickham2010layered}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{To render a }\textit{\DIFadd{slab}} \DIFadd{layer, first, the }\texttt{\DIFadd{layer}} \DIFadd{applies an initial set of aesthetic mappings (}\cref{fig:implementation}\DIFadd{A), defining (at least) positional variables like $x$/$y$ and distribution mappings like $x_\textsc{dist}$/$y_\textsc{dist}$. Then, }\texttt{\DIFadd{stat\_slab}} \DIFadd{(}\cref{fig:implementation}\DIFadd{B) evaluates the distributional functions (the density and }\textsc{\DIFadd{cdf}}\DIFadd{) and intervals for each distribution in the $x_\textsc{dist}$ (or $y_\textsc{dist}$) aesthetic. It determines reasonable limits within which to draw each distribution (the exact limits if finite, and the $0.001$th or $0.999$th quantile by default if infinite) and creates a grid of $x$ (or $y$) values to evaluate the functions at. Several special cases must be handled, including constant distributions (rendered as a point mass) and discrete distributions (as a histogram; see the end of }\cref{sec:use-in-the-wild}\DIFadd{). It then creates a data frame containing values of $x$,  $f(x)$ (in a column named }\texttt{\DIFadd{pdf}}\DIFadd{),}\footnote{\DIFadd{This is also where corrections to the density must be made, depending on the $x$ (or $y$) scale function; see }\cref{sec:jacobians}\DIFadd{.}} \DIFadd{$F(x)$ (}\texttt{\DIFadd{cdf}}\DIFadd{), and $\gamma$ (}\texttt{\DIFadd{.width}}\DIFadd{)---the mass of the smallest interval containing $x$. These new columns are called }\textit{\DIFadd{computed variables}} \DIFadd{in }\textit{\DIFadd{ggplot2}}\DIFadd{.
}

\DIFadd{The }\texttt{\DIFadd{layer}} \DIFadd{then applies a second set of aesthetic mappings (}\cref{fig:implementation}\DIFadd{C): those which depend on computed variables (as indicated by wrapping them in }\texttt{\DIFadd{after\_stat()}} \DIFadd{or by using the probability expression mini-}\textsc{\DIFadd{dsl}}\DIFadd{; see }\cref{foot:after-stat}\DIFadd{). Then, }\texttt{\DIFadd{geom\_slab}} \DIFadd{(}\cref{fig:implementation}\DIFadd{D) renders each slab using one of two algorithms, depending on its }\texttt{\DIFadd{fill\_type}} \DIFadd{parameter: }\texttt{\DIFadd{segments}} \DIFadd{or }\texttt{\DIFadd{gradient}}\DIFadd{. Prior to R 4.1, the R graphics engine did not have proper gradient support, so the only available algorithm was }\texttt{\DIFadd{segments}} \DIFadd{(depicted in }\cref{fig:implementation}\DIFadd{D), which sub-divides slabs into blocks of consecutive $x$ (or $y$) values with the same appearance, interpolating $x$, $y$, and }\textit{\DIFadd{thickness}} \DIFadd{values at cutpoints between blocks. This algorithm remains well-suited for fills with sharp cuts, such as }\textsc{\DIFadd{rope}}\DIFadd{s or small numbers of intervals. Alternatively, as of R 4.1, with proper gradient support in some R graphics output formats (e.g. }\textsc{\DIFadd{svg}} \DIFadd{and }\textsc{\DIFadd{pdf}}\DIFadd{)~\mbox{%DIFAUXCMD
\cite{murrell2022vectorised}}\hskip0pt%DIFAUXCMD
, }\textit{\DIFadd{ggdist}} \DIFadd{can output high-quality color gradients. The }\texttt{\DIFadd{gradient}} \DIFadd{algorithm does this by rendering each slab as a polygon with a linear gradient fill, using the $x$ (or $y$) positions as control points on the gradient. This algorithm is best-suited to color gradient density plots.
}

\DIFadd{In support of the above pipeline, }\textit{\DIFadd{ggdist}} \DIFadd{includes extensive utilities for manipulating distributions (some of which are exposed in its }\textsc{\DIFadd{api}}\DIFadd{), including functions for calculating various interval types and for determining various distributional properties (e.g. detecting discrete or constant distributions and determining distribution limits). As the basic structure of the pipeline in }\textit{\DIFadd{ggplot2}} \DIFadd{should be similar to other grammar of graphics systems, my hope is that such systems could easily adopt a formalism like the one }\DIFaddend I've \DIFdelbegin \DIFdel{learned a few things that may be useful for others tackling uncertainty visualization to consider.
}\DIFdelend \DIFaddbegin \DIFadd{described, allowing them to support a variety of uncertainty visualizations through a distributional approach.
%DIF > as of this writing is one of the only R packages to do so (even base \textit{ggplot2} has not implemented it yet).
}\DIFaddend 

\DIFdelbegin \subsection{\DIFdel{Distributional notation makes uncertainty visualization much less annoying}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend %DIF > The \texttt{segments} algorithm (depicted in the figure) splits 


%DIF > ; to do this, it must determine a reasona
\DIFaddbegin 

%DIF > , and \texttt{geom\_slab}, which constructs the low-level graphical objects used to display the slab. 



%DIF > The interested implementer might refer to 

%DIF > Add bit about gradient rendering and reference Paul Murrell's article

\section{\DIFadd{Reflections and lessons learned}}

%DIF > \textit{ggdist} has been under development in some form or another for about six--seven years, and over that time has evolved through user feedback and several waves of refactoring and rewriting. Through all that, I hope I've learned a few things that may be useful for others tackling uncertainty visualization to consider.

\subsection{\DIFadd{Distributional notation makes uncertainty}\\\DIFadd{visualization much less annoying}}

\DIFaddend %\textbf{TODO} distributions as core to uncertainty vis, unifying Bayes/freq, power of the syntax itself (making parameters into data, vectorization of arguments to create distributions made the dist+args syntax obsolete, etc)
...
\DIFdelbegin \footnote{\DIFdel{Pedants get one properly typeset ``formul\ae'' per paper. There it was.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend %DIF > \footnote{Pedants get one properly typeset ``formul\ae'' per paper. There it was.} 
...
A continual source of tension in the design of \textit{ggdist} has been the balance between abstraction and learnability. At its base, \textit{ggdist} actually only has three geometries, all of which are composite \textit{meta}-geometries: (1) a \textit{slabinterval} geometry, consisting of \DIFaddbegin \DIFadd{a }\DIFaddend \textit{slab}, \textit{point}, and \textit{interval}\DIFdelbegin \DIFdel{sub-geometries}\DIFdelend ; (2) a \textit{lineribbon} geometry, consisting of \DIFaddbegin \DIFadd{a }\DIFaddend \textit{line} and \textit{ribbon}\DIFdelbegin \DIFdel{sub-geometries}\DIFdelend ; and (3) a \textit{dotsinterval} geometry, consisting of \textit{dots}, \DIFaddbegin \DIFadd{a }\DIFaddend \textit{point}, and \DIFaddbegin \DIFadd{an }\DIFaddend \textit{interval}\DIFdelbegin \DIFdel{sub-geometries}\DIFdelend . All other geometries in \textit{ggdist} are \textit{shortcut} geometries, constructed using some combination of default parameters and/or aesthetic mappings applied to one of those three \textit{meta}-geometries.

In principle, \textit{ggdist} could have only those three geometries\DIFdelbegin \DIFdel{, but in practice}\DIFdelend \DIFaddbegin \DIFadd{; in practice, }\DIFaddend this would not be as usable. First, \DIFdelbegin \DIFdel{many specific }\DIFdelend \DIFaddbegin \DIFadd{certain }\DIFaddend combinations of options are often used together \DIFdelbegin \DIFdel{, so shortcuts for those combinations can save users }\DIFdelend \DIFaddbegin \DIFadd{to create useful geometries; shortcuts for these save }\DIFaddend time. Second, many users will not delve into the depths of \textit{ggdist}'s options to learn how to create each combination they need, \DIFdelbegin \DIFdel{and find it easier to remember the name of a shortcut}\DIFdelend \DIFaddbegin \DIFadd{so shortcuts aid learnability and discoverability}\DIFaddend . \textit{ggdist}'s \textit{shortcut} geometries include ones for intervals, eye plots (mirrored slabs with a \textit{pointinterval}), complementary \textsc{cdf} plots, and gradient plots (See \cref{fig:teaser}A). 
I have found users appreciate these shortcuts, and often ask for more: a raincloud plot shortcut (\cref{fig:teaser}E) is a common request, for example.\DIFaddbegin \footnote{\DIFadd{A request I am unlikely to fulfill, as the complexity of specifying options and aesthetics for such a bloated composite geometry---with }\textit{\DIFadd{point}}\DIFadd{, }\textit{\DIFadd{interval}}\DIFadd{, }\textit{\DIFadd{slab}}\DIFadd{, and }\textit{\DIFadd{dots}} \DIFadd{sub-geometries!---means the shortcut is unlikely to be much easier to use than just specifying a }\textit{\DIFadd{slabinterval}} \DIFadd{alongside a }\textit{\DIFadd{dots}} \DIFadd{geometry.}}
\DIFaddend 

\DIFdelbegin \DIFdel{In general, I 've found users often request shortcuts for creating specific chart types to solve a problem at hand, which is a not unreasonable motivation. However, I }\DIFdelend %DIF > Users often request shortcuts for creating specific chart types to solve a problem at hand, which is a not-unreasonable motivation. 
\DIFaddbegin \DIFadd{I }\DIFaddend have found it productive to hold off on implementing every \DIFdelbegin \DIFdel{such request: }\DIFdelend \DIFaddbegin \DIFadd{requested shortcut. }\DIFaddend I usually wait until I can figure out how solving a particular problem (e.g., creating Helske-esque violin-interval plots) can be done within the \DIFdelbegin \DIFdel{broader }\DIFdelend distributional framework in a way that expands the expressiveness of \textit{ggdist} (e.g., allowing interval masses to be mapped onto slab aesthetics). \DIFdelbegin \DIFdel{I have also found that }\DIFdelend \DIFaddbegin \DIFadd{Meanwhile, }\DIFaddend extensive documentation and examples \DIFdelbegin \DIFdel{can temper }\DIFdelend \DIFaddbegin \DIFadd{tempers }\DIFaddend the need for quite so many \textit{explicit} shortcuts; an easy-to-adapt example in the documentation acts as a sort of shortcut itself. \textit{ggdist} has extensive long-form \textit{vignettes} that showcase myriad plot types with simple examples. This tension between abstraction and learnability is common to grammar of graphics toolkits: if the ``purest'' form of the grammar of graphics has no chart types at all (per Wilkinson~\cite{wilkinson2012grammar}), but designers often think in terms of chart types~\cite{pu2023inpractice}, how do we best meet them in the middle?
...
\DIFdelbegin \footnote{\DIFdel{Let's be honest, Xiaoying did all the hard work.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend %DIF > \footnote{Let's be honest, Xiaoying did all the hard work.} 
termed a \textit{tight coupling} \DIFdelbegin \DIFdel{of data transformation and visualization specification }\DIFdelend in a study of \DIFdelbegin \DIFdel{expert }\DIFdelend \textit{ggplot2} users~\cite{pu2023inpractice}: \DIFdelbegin \DIFdel{the data processing }\DIFdelend \DIFaddbegin \DIFadd{data pre-processing }\DIFaddend (calculating the density) is tightly coupled with the visualization \DIFdelbegin \DIFdel{spec }\DIFdelend \DIFaddbegin \DIFadd{specification }\DIFaddend (the scale transformation), and these must be kept in sync. Moving the \DIFdelbegin \DIFdel{specification }\DIFdelend \DIFaddbegin \DIFadd{calculation }\DIFaddend of densities into the visualization \DIFdelbegin \DIFdel{spec }\DIFdelend \DIFaddbegin \DIFadd{specification }\DIFaddend itself is one way to ensure this, eliminating a whole class of \DIFaddbegin \DIFadd{potential }\DIFaddend errors. 
